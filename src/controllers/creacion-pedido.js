...
async function generateUniqueCode(fecha, connection) {
    ...
    const rows = await new Promise((resolve, reject) => {
        connection.query(countQuery, countParams, (err, results) => {
            if (err) {
                reject(err);
            } else {
                resolve(results);
...
module.exports.create = async (req, res) => {
    ...
    try {
        ...
        const codigo = await generateUniqueCode(fecha, connection).catch(error => { throw error });
        ...
        const pedidoResult = await new Promise((resolve, reject) => {
            connection.query(pedidoQuery, pedidoParams, (err, results) => {
                if (err) {
                    return reject(err);
                }
                resolve(results);
        ...
        const materialQueries = materiales.map(material => {
            ...
            return new Promise((resolve, reject) => {
                connection.query(materialQuery, materialParams, (err, results) => {
                    if (err) {
                        return reject(err);
                    }
                    resolve(results);
